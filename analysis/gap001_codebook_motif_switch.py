...
same placement and strand for T and C. PWM changes are sequence-model signals,
not evidence of in-vivo occupancy, target-gene regulation, or causality.
VPH expression is used only as a feasibility filter.
"""
...
def best_variant_overlapping_switch(tf: str, pwm: np.ndarray, ref_seq: str, alt_seq: str, variant_index: int) -> MotifSwitch:
    length = len(pwm)
    ...
    for start in range(min_start, max_start + 1):
        ...
        for strand, rw, aw in (("+", ref_forward, alt_forward), ("-", reverse_complement(ref_forward), reverse_complement(alt_forward))):
            ...
            candidates.append(MotifSwitch(tf, length, strand, start - variant_index, rs, ac, rn, an, delta, abs(delta), max(rn, an), direction, (rn >= 0.75) != (an >= 0.75), rw, aw))
    if not candidates:
    ...
    with h5py.File(path, "r") as h5:
        ...
        for tf in human_tfs:
            ...
            for symbol in mouse_symbol_candidates(tf):
                ...
                if len(hits) == 1:
                    match = (symbol, int(hits[0])); break
            if match is None:
                vectors[tf] = np.zeros(shape[1], dtype=bool); resolved[tf] = None
            else:
                symbol, idx = match
...
def expression_summary(input_root: Path, tfs: list[str]) -> list[dict[str, object]]:
    ...
    for sample, sex in SAMPLES.items():
        ...
        for tf in tfs:
            ...
            rec["cells"] += n_cells; rec["positive"] += pos
            rec[f"{sex}_cells"] += n_cells; rec[f"{sex}_positive"] += pos
            if resolved[tf]: rec["symbols"].add(resolved[tf])
    rows = []
    for tf in tfs:
        rec = aggregate[tf]
        rows.append({
...
def main() -> None:
    ...
    p.add_argument("--motif-dir", type=Path, required=True); p.add_argument("--sequence-json", type=Path, required=True)
    p.add_argument("--variant-index", type=int, required=True); p.add_argument("--ref", default="T"); p.add_argument("--alt", default="C")
    p.add_argument("--expression-root", type=Path); p.add_argument("--top-expression", type=int, default=30); p.add_argument("--output-dir", type=Path, required=True)
    args = p.parse_args(); args.output_dir.mkdir(parents=True, exist_ok=True)
    ref_seq = str(json.loads(args.sequence_json.read_text()).get("dna", "")).upper()
    if not ref_seq or any(b not in BASES for b in ref_seq): raise ValueError("UCSC sequence JSON missing clean A/C/G/T dna")
    if ref_seq[args.variant_index] != args.ref: raise ValueError(f"reference base mismatch at variant: expected {args.ref}, got {ref_seq[args.variant_index]}")
    ...
    # Expression feasibility must include every threshold crossing, even if its
    # raw delta rank falls below the nominal top-N list.
    selected_tfs = list(dict.fromkeys([x.tf for x in switches[:args.top_expression]] + [x.tf for x in crossings] + [x.tf for x in strong]))
    ...
    integrated = []
    rank_by_tf = {x.tf: i + 1 for i, x in enumerate(switches)}
    for sw in switches[:args.top_expression]:
        row = {"rank": rank_by_tf[sw.tf], **asdict(sw)}
        row.update({f"expr_{k}": v for k, v in expr_by_tf.get(sw.tf, {}).items() if k != "tf"}); integrated.append(row)
    write_csv(args.output_dir / "top_integrated_candidates.csv", integrated)

    crossing_rows = []
    for sw in crossings:
        row = {"rank": rank_by_tf[sw.tf], **asdict(sw)}
        row.update({f"expr_{k}": v for k, v in expr_by_tf.get(sw.tf, {}).items() if k != "tf"}); crossing_rows.append(row)
    write_csv(args.output_dir / "threshold_crossings.csv", crossing_rows)

    report = ["# GAP-001 Codebook allele motif-switch scan", "", "**Status: sequence-model discovery screen; not occupancy or causality.**", "",
              f"- Motifs scanned: **{len(switches)}**", f"- Exact edit: **{args.ref}>{args.alt}** at supplied variant index {args.variant_index}",
              f"- Motifs with >=0.75 normalized site strength on either allele: **{len(strong)}**", f"- Heuristic 0.75 threshold crossings: **{len(crossings)}**", "",
              "## Heuristic 0.75 threshold crossings", "", "|rank|TF|direction|ref|alt|abs Δnorm|VPH +%|male +%|female +%|", "|---:|---|---|---:|---:|---:|---:|---:|---:|"]
    for row in crossing_rows:
        report.append(f"|{row['rank']}|{row['tf']}|{row['direction']}|{row['ref_normalized']:.4f}|{row['alt_normalized']:.4f}|{row['abs_delta_normalized']:.4f}|{float(row.get('expr_vph_positive_pct',0)):.3f}|{float(row.get('expr_male_positive_pct',0)):.3f}|{float(row.get('expr_female_positive_pct',0)):.3f}|")
    report += ["", "## Top allele-sensitive motifs", "", "|rank|TF|direction|abs Δnorm|max strength|strand|offset|VPH +%|mouse symbol|", "|---:|---|---|---:|---:|:---:|---:|---:|---|"]
    for row in integrated[:20]:
        report.append(f"|{row['rank']}|{row['tf']}|{row['direction']}|{row['abs_delta_normalized']:.4f}|{row['max_site_strength']:.4f}|{row['strand']}|{row['start_offset']}|{float(row.get('expr_vph_positive_pct',0)):.3f}|{row.get('expr_mouse_symbols','')}|")
    report += ["", "## Interpretation boundary", "", "- PWM score change means the DNA substitution changes a sequence model for TF recognition.", "- VPH transcript detection is only a feasibility prerequisite; absence can demote a direct adult-VPH interpreter, presence cannot establish binding.", "- The 0.75 crossing is a declared heuristic for triage, not a biochemical affinity threshold.", "- No candidate is promoted to causal status without allele-specific occupancy and perturbation in the relevant PH cell state."]
    text = "\n".join(report) + "\n"; (args.output_dir / "report.md").write_text(text); print(text)
